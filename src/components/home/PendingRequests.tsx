...
import { FriendshipStatus, User, CreateFriendshipInput, UpdateFriendshipInput, UpdateFriendshipMutation} from "../../API";
...
const handleAccept = async (owner: string) => {
  ...
  const checkPendings = await API.graphql(graphqlOperation(listFriendships, { 
    filter: {
      and: [
    ...
    await API.graphql(
      graphqlOperation(updateFriendship, { input: updateInput })
    ).then(
      //@ts-ignore
      async (result) => {
        console.log(result)
...
const columns: GridColDef[] = [
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <div>
        <Button size='small' variant="contained" color="primary" onClick={() => handleAccept(row.owner)}>
          Accept
        </Button>
        <Button size='small' variant="contained" color="primary" onClick={() => console.log("rejected")}
          sx={{ ml: 5 }}
        >
          Reject
        </Button>
...
const PendingRequests = () => {
  ...
  const handlePendingRequests = async () => {
    ...
    if(checkPendings.data.listFriendships.items.length > 0) {
      ...
      userCheckArray.map(async (item, index) => {
        ...
        const user = await API.graphql(graphqlOperation(getUser, { owner: item }));
  ...
  return (
    <Card>
      ...
      <DataGrid autoHeight hideFooter getRowId={(row) => row.owner} rows={userArray} columns={columns} disableSelectionOnClick pagination={undefined} />
    </Card>
  )
}
